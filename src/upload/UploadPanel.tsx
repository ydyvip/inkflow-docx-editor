...
interface UploadPanelProps {
  onParsed: (json: any, fileName: string, warnings: string[], comments: DocxComment[]) => void;
}
    ...
    <div class="upload-panel">
      ...
      >
        ...
        <div class="dropzone-title">
          {status() === "parsing" ? "正在解析 DOCX…" : "拖拽 .docx 文件到此处，或点击选择"}
        </div>
        <div class="dropzone-sub">DOCX → mammoth → HTML(中间态) → DOM → ProseMirror JSON</div>
        {status() === "error" && <div class="dropzone-error">解析失败：{error()}</div>}
      </div>
