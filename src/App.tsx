...
function App() {
  ...
  const handleParsed = (json: any, name: string, msgs: string[], parsedComments: DocxComment[]) => {
    setDocJson(json);
    setFileName(name);
    setWarnings(msgs);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      showToast(`导出失败：${err instanceof Error ? err.message : String(err)}`, 3000);
    } finally {
      setBusy("");
    }
  };
    ...
    <div class="app-shell">
      <header class="app-header">
        ...
        <nav class="pipeline" aria-label="处理流程">
          <span class={`pipeline-step${hasDoc() ? " is-done" : stage() === "upload" ? " is-active" : ""}`}>
            <span class="pipeline-dot" />
            上传 DOCX
          </span>
          <span class="pipeline-line" />
          <span class={`pipeline-step${stage() !== "upload" ? " is-active" : ""}`}>
            <span class="pipeline-dot" />
            结构化 JSON
          </span>
          <span class="pipeline-line" />
          <span class={`pipeline-step${pulseExport() ? " is-active is-pulse" : ""}`}>
            <span class="pipeline-dot" />
            导出 DOCX
          </span>
        </nav>
        ...
        <div class="header-actions">
          ...
          <button type="button" class="export-btn" disabled={!hasDoc() || busy() !== ""} onClick={handleExport}>
      ...
      <Show when={warnings().length > 0 && stage() !== "upload"}>
        <div class="warning-bar">
          还原提示（结构优先，样式可能与原文件存在差异）：{warnings().slice(0, 2).join("；")}
