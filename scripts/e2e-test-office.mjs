...
try {
  ...
  if ((await h9.count()) < 1) throw new Error("未能创建 H9 标题（<h9> 元素不存在）");
  if (!(await h9.first().innerText()).includes("标题层级 9")) throw new Error("H9 标题文本不正确");
  const h9Display = await h9.first().evaluate((el) => getComputedStyle(el).display);
  if (h9Display !== "block") throw new Error("H9 未按块级渲染，display=" + h9Display);
  log("H1/H5/H9 headings created; H9 renders as a block-level element via CSS fallback");

  // ---- 2. Outline shown in both Editor and Preview ----
  const editOutlineCount = await page.locator(".outline-item button").count();
  if (editOutlineCount < 3) throw new Error("编辑器目录树条目数不对: " + editOutlineCount);
  await page.getByRole("tab", { name: "预览" }).click();
  await page.waitForSelector(".preview-page", { timeout: 5000 });
  const previewOutlineCount = await page.locator(".outline-item button").count();
  if (previewOutlineCount < 3) throw new Error("预览里目录树条目数不对: " + previewOutlineCount);
  await page.locator(".outline-item button").nth(1).click();
  await page.waitForSelector(".pm-highlight", { timeout: 3000 });
  log(`outline shown in both Editor (${editOutlineCount}) and Preview (${previewOutlineCount}); preview click-to-jump highlights too`);
  ...
  await page.locator(".toolbar-format .toolbar-select").first().selectOption("Arial");
  await page.waitForTimeout(80);
  await page.locator(".toolbar-select-narrow").first().selectOption("24");
  await page.waitForTimeout(80);
  await page.locator('.toolbar-color input[type="color"]').first().fill("#cc0000");
  await page.waitForTimeout(80);
  await page.getByTitle("下划线").click();
  await page.waitForTimeout(80);
  await page.getByTitle("删除线").click();
  await page.waitForTimeout(80);
  await page.locator(".toolbar-format .toolbar-select-narrow").nth(1).selectOption("yellow");
  await page.waitForTimeout(150);
  ...
  const hasUnderline = (await page.locator(".ProseMirror u .docx-style-run").count()) > 0;
  if (!hasUnderline) throw new Error("下划线未生效");
  log("font family/size/color/highlight/underline/strike all applied via toolbar: " + styledStyle);
  ...
  if (!paraStyle || !paraStyle.includes("center") || !paraStyle.includes("margin-left") || !paraStyle.includes("line-height")) {
  ...
  const stillHasStyledText = (await page.locator(".ProseMirror").innerText()).includes("格式化测试文字");
  if (!stillHasStyledText) throw new Error("此前的字体格式段落在后续编辑中被破坏/丢失");
  ...
  log(`insert row + column: cell count ${initialCellCount} -> ${afterInsertCount}`);

  await cells.first().click();
  await page.waitForTimeout(80);
  await page.getByRole("button", { name: "表头行" }).click();
  await page.waitForTimeout(100);
  const headerCellCount = await table.locator("tr").first().locator("th").count();
  ...
  await page.waitForTimeout(100);
  const firstCellStyle = await table.locator("tr").first().locator("th, td").first().getAttribute("style");
  if (!firstCellStyle || !firstCellStyle.includes("rgb(0, 204, 102)")) throw new Error("单元格底色未生效: " + firstCellStyle);
  ...
  await dragSelectCells(row2.locator("td, th").nth(0), row2.locator("td, th").nth(1));
  const hasCellSelection = (await page.locator(".ProseMirror .selectedCell").count()) > 0;
  if (hasCellSelection) {
    await page.getByRole("button", { name: "合并单元格" }).click();
    await page.waitForTimeout(100);
    const rowCellsAfter = await row2.locator("td, th").count();
    if (rowCellsAfter >= rowCellsBefore) throw new Error("合并单元格未减少该行单元格数");
    log(`merged two cells in a row: ${rowCellsBefore} -> ${rowCellsAfter} cells`);
  } else {
    log("⚠ drag cell-range selection did not register (environment/timing) — skipping merge assertion, not a hard failure");
  }
  ...
  if (bytes.subarray(0, 2).toString("latin1") !== "PK") throw new Error("导出文件不是合法 docx");
  log("exported office-grade document to " + outPath + `, size=${bytes.length} bytes`);
