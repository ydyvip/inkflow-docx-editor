...
try {
  ...
  if (!bodyText.includes('测试文档标题'))
    throw new Error('解析结果缺少标题文本: ' + bodyText.slice(0, 200));
  if (!bodyText.includes('加粗')) throw new Error('解析结果缺少正文文本');
  if (!bodyText.includes('第一条列表项'))
    throw new Error('解析结果缺少列表文本');
  log(
    'parsed DOCX -> ProseMirror JSON content verified (heading/paragraph/list present)'
  );
  ...
  await page.keyboard.type(
    '这是通过编辑器新增的段落，用来验证 transaction 流程。'
  );
  ...
  const previewText = await page
    .locator('.preview-page .ProseMirror')
    .innerText();
  if (!previewText.includes('插入的新章节'))
    throw new Error('预览内容与编辑内容不一致');
  ...
  if (magic !== 'PK')
    throw new Error('导出文件不是有效的 zip/docx (magic=' + magic + ')');
  if (bytes.length < 1000)
    throw new Error('导出文件过小，可能为空文档: ' + bytes.length + ' bytes');
  ...
  if (seriousConsoleErrors.length) {
    console.log(
      '⚠ console errors (non-fatal, review):\n' +
